...
export default function MainNavbar() {
  ...
  return (
    <>
      <header className="h-16 text-[15px] fixed inset-0 flex-center bg-[#0A1D3A] z-[1000]">
        <nav className="px-3.5 flex-center-between w-full max-w-7xl mx-auto">
          <div className="flex-center gap-x-3 z-[999] relative">
            <Link
              to="/"
        ...
        <motion.div
          ref={drawerRef}
          className="fixed left-0 right-0 top-16 overflow-y-auto h-full bg-[#0A1D3A] text-white p-6 pb-20 z-[1001]"
          initial={{ x: "-100%" }}
          animate={{ x: isOpen ? "0%" : "-100%" }}
          transition={{ duration: 0.3, ease: "easeInOut" }}
        >
